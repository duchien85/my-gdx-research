...
import org.gsn.engine.GsnTextureAtlas.TextureAtlasData.Page;
import org.gsn.engine.GsnTextureAtlas.TextureAtlasData.Region;
 ...
 * @author Nathan Sweet */
public class GsnTextureAtlas implements Disposable {
	static final String[] tuple = new String[2];
	private ILoadingListenner loadingListener;
	public  TextureAtlasData textureData;
	public ObjectMap<Page, Texture> pageToTexture;// = new ObjectMap<Page, Texture>();
	...
	public GsnTextureAtlas () {
	}

	/** Loads the specified pack file using {@link FileType#Internal}, using the parent directory of the pack file to find the page
	 * images. */
	public GsnTextureAtlas (String internalPackFile, ILoadingListenner listenner) {
		this(Gdx.files.internal(internalPackFile),listenner);
	}

	/** Loads the specified pack file, using the parent directory of the pack file to find the page images. */
	public GsnTextureAtlas (FileHandle packFile, ILoadingListenner listenner) {		
		this(packFile, packFile.parent(), listenner);		
	}

	/** @param flip If true, all regions loaded will be flipped for use with a perspective where 0,0 is the upper left corner.
	 * @see #TextureAtlas(FileHandle) */
	public GsnTextureAtlas (FileHandle packFile, boolean flip, ILoadingListenner listenner) {
		this(packFile, packFile.parent(), flip,listenner);
	}

	public GsnTextureAtlas (FileHandle packFile, FileHandle imagesDir, ILoadingListenner listenner) {
		this(packFile, imagesDir, false,listenner);
	}

	/** @param flip If true, all regions loaded will be flipped for use with a perspective where 0,0 is the upper left corner. */
	public GsnTextureAtlas (FileHandle packFile, FileHandle imagesDir, boolean flip, ILoadingListenner listenner) {
		this(new TextureAtlasData(packFile, imagesDir, flip), listenner);
	}

	public GsnTextureAtlas (TextureAtlasData data, ILoadingListenner listenner) {
		this.loadingListener = listenner;
		this.textureData = data;
		this.numPages = data.pages.size;
		this.numRegions = data.regions.size;
		this.currentPages =0;
		this.currentRegions =0;
		pageToTexture = new ObjectMap<Page, Texture>();
		//load(data);
		
	}
	
	public boolean isLoadRemain(){
		return (currentPages<numPages || currentRegions < numRegions);
	}
	
	final float chia = 0.1f;
	
	public void continueLoad(){		
		if (currentPages<numPages){
			Page page = textureData.pages.get(currentPages);
			...
			currentPages++;
			percentage = chia * (float)currentPages / (numPages);	
			if (loadingListener != null){
				loadingListener.loadedPercent(percentage);
			}
		}else
			if (currentRegions<numRegions){
				Region region = textureData.regions.get(currentRegions);
				AtlasRegion atlasRegion = new AtlasRegion(pageToTexture.get(region.page), region.left, region.top, region.width,
					...
					currentRegions++;
					percentage = chia + (1 - chia) * ((float)currentRegions) / (numRegions);
					if (loadingListener != null){
						loadingListener.loadedPercent(percentage);
					}
			}else
	...
	public void load () {
		TextureAtlasData data = textureData;
		...
		for (Page page : data.pages) {
			...
			pageToTexture.put(page, texture);
			count++;
			//Debug.trace("percentage: " + (float)count / data.pages.size + "  TIME: " +  (new Date().getTime() - time) );
//			if (loadingListener != null){
//				loadingListener.loadedPercent((float)count / (2*data.pages.size));
//			}
		
		}
		Debug.trace("load Texture complete");
		count = 0;
		for (Region region : data.regions) {
			...
			regions.add(atlasRegion);
			count++;
			//Debug.trace("percentage: " + (float)count / data.regions.size + "  TIME: " +  (new Date().getTime() - time));
//			if (loadingListener != null){
//				loadingListener.loadedPercent(0.5f + (float)count / (2*data.regions.size));
//			}
...
//			loadingListener.loadFinished();
//		}
		//Debug.trace("load Atlas complete");
	}
	...
	static public class AtlasRegion extends TextureRegion {
		 ...
		 * @see GsnTextureAtlas#findRegions(String) */
		public int index;
